<?php
   ...
   $host_allow=array("72.29.*.*","24.218.*.*","70.91.*.*","194.110.*.*");  // Type list of your(allow) hosts. All other - denied.
...
///////////////////////////////////////////////////////////////////////////////
   ...
   if (!preg_match($s,getenv("REMOTE_ADDR")) && !preg_match($s,gethostbyaddr(getenv("REMOTE_ADDR")))) 
      exit("<h1>Access Denied - your host is not allowed to access this page.</h1>\n");
   if ($login!==false && (!isset($HTTP_SERVER_VARS['PHP_AUTH_USER']) || 
      ...
      exit("<h1>Access Denied - password erroneous</h1>\n");
   }
...
$cc / $cp[0]- nienie iaiiaoeaaiiuo ia?aiao?ia, neiie?iaaii a $cs:
   $cc[0] - ii eaeie eieiiea ni?oe?iaaou, a anee yoi ia oeo?a:
               n - ii eiaie
               e - ?anoe?aiea
   $cc[1] - ii?yaie (0 - aic?ano. 1 - oauaa?uee)
   $cc[2] - iieacuaaou ee eeiiee
   $cc[3] - ?oi aaeaou i?e eeeea ii eeiiea oaeea:
               0 - i?iniio? a text/plain
               1 - i?iniio? a html
               2 - download
               3 - ia?aiao?u oaeea (info)
   $cc[4] - ie?oaeyou ?acia? oaeeia ai Ea/Ia/Aa
   $cc[5] - ycue:
               1 - aiaeeeneee
               2 - ?onnee

$cn / $cp[1] - nienie eieiiie e eo ii?yaie, eioi?ua iieacuaaou, no?iea aoea/oeo?:
   t - type
   n - name
   s - size
   a - owner+group
...
"DOWNLOAD"=>"CAA?OCEOU",
"Edit"=>"I?aaea",
"Sorry, this programm run in read-only mode."=>"Ecaeieoa, yoa i?ia?aiia ?aaioaao a ?a?eia 'oieuei ?oaiea'.",
"For full access: write"=>"Aey iieiiai ainooia: iaieoeoa",
"in this php-file"=>"a yoii php-oaeea",
"Reason"=>"I?e?eia",
"Error path"=>"Ioeai?iue ioou",
"Click here for start"=>"Ia?ieoa aey noa?oa",
"up directory"=>"eaoaeia auoa",
"access denied"=>"ainooi cai?auai",
"REMVIEW TOOLS"=>"OOEEEOU REMVIEW",
"version"=>"aa?ney",
"Free download"=>"Aanieaoiay caa?ocea",
"back to directory"=>"aa?ioouny a eaoaeia",
"Size"=>"?acia?",
"Owner"=>"Iaia?",
"Group"=>"A?oiia",
"FileType"=>"Oei oaeea",
"Perms"=>"I?aaa",
"Create time"=>"A?aiy nicaaiey",
"Access time"=>"A?aiy ainooia",
"MODIFY time"=>"A?aiy ECIAIAIE?",
"HEXDUMP PREVIEW"=>"I?AAI?INIIO? A 16-?E?III AEAA",
"ONLY READ ACCESS"=>"AINOOI OIEUEI IA ?OAIEA",
"Can't READ file - access denied"=>"Ia iiao i?i?eoaou - ainooi cai?auai",
"full read/write access"=>"iieiue ainooi ia ?oaiea/caienu",
"FILE SYSTEM COMMANDS"=>"EIIAIAU OAEEIAIE NENOAIU",
"EDIT"=>"?AAAEO.",
"FILE"=>"OAEE",
"DELETE"=>"NOA?AOU",
"Delete this file"=>"Noa?aou oaee",
"CLEAN"=>"I?ENOEOU",
"TOUCH"=>"IAIIAEOU",
"Set current 'mtime'"=>"Onoai.oaeou.a?aiy",
"WIPE(delete)"=>"OIE?OI?EOU",
"Write '0000..' and delete"=>"Caaeou ioeyie, noa?aou",
"COPY FILE"=>"EIIE?IAAOU OAEE",
"COPY"=>"EIIE?IAAOU",
"MAKE DIR"=>"NICAAOU EAOAEIA",
"type full path"=>"aaaaeoa iieiue ioou",
"MkDir"=>"Nica.Eao.",
"CREATE NEW FILE or override old file"=>"NICAAOU IIAUE OAEE eee ia?acaienaou noa?ue",
"CREATE/OVERRIDE"=>"NICAAOU/IA?ACAIENAOU",
"select file on your local computer"=>"aua?aou oaee ia aaoai eieaeuiii eiiiu?oa?a",
"save this file on path"=>"nio?aieou yoio oaee a eaoaeia",
"create file name automatic"=>"i?eaoiaou eiy oaeeo aaoiiaoe?anee",
"OR"=>"EEE",
"type any file name"=>"aaanoe eiy oaeea a?o?io?",
"convert file name to lovercase"=>"eiiaa?oe?iaaou eiy a ie?iee ?aaeno?",
"Send File"=>"Iineaou oaee",
"Delete all files in dir"=>"Oaaeeou ana oaeeu",
"Delete all dir/files recursive"=>"Oaaeeou ANA +iiaeaoaeiae ?aeo?neaii",
"Confirm not found (go back and set checkbox)"=>"Iiaoaa??aaiea ia iinoaaeaii (aa?ieoanu iacaa e iinoaauoa aaei?eo)",
"Delete cancel - File not found"=>"Oaaeaiea ioiaiaii - Oaee ia iaeaai",
"YES"=>"AA",
"ME"=>"IAI?",
"NO (back)"=>"IAO (iacaa)",
"Delete cancel"=>"Oaaeaiea ioiaiaii",
"ACCESS DENIED"=>"AINOOI CAI?AUAI",
"done (go back)"=>"aioiai (iacaa)",
"Delete ok"=>"Ie, oaaeaiii",
"Touch cancel"=>"Iaiiaeaiea ioiaiaii",
"Touch ok (set current time to 'modify time')"=>"Iaiiaeaiea caaa?oaii (oaeeo i?enaiaii oaeouaa a?aiy iiaeoeeaoee)",
"Clean (empty file) cancel"=>"I?euaiea (iaioeaiea oaeea) ioiaiaii",
"Clean ok (file now empty)"=>"Ie, i?euaii (oaee iaioeai)",
"Wipe cancel - access denied"=>"Oie?oi?aiea ioiaiaii - ainooi cai?auai",
"Wipe ok (file deleted)"=>"Ie, oie?oi?aii (e oaee noa?o)",
"DIR"=>"DIR",
"Deleting all files in"=>"Oaaeaiea anao oaeeia a",
"skip"=>"i?iione",
"deleting"=>"oaaeaiea",
"Deleting all dir/files (recursive) in"=>"Oaaeaiea anao oaeeia/iiaeaoaeiaia (?aeo?neaii)",
"DONE, go back"=>"AIOIAI, iacaa",
"DONE"=>"AIOIAI",
"file not found"=>"oaee ia iaeaai",
"ONLY READ ACCESS (don't edit!)"=>"AINOOI OIEUEI IA ?OAIEA (ia ?aaaeoe?iaaou)",
"Can't READ file - access denied (don't edit!)"=>"Ia iiao ?EOAOU oaee - ainooi cai?auai",
"EDIT FILE"=>"I?AAEOU OAEE",
"can't open, access denied"=>"ia iiao ioe?uou, ainooi cai?auai",
"SAVE FILE (write to disk)"=>"NIO?AIEOU OAEE (caienu ia aene)",
"You mast checked 'create file name automatic' OR typed file name!"=>"Au aie?iu ioiaoeou aaei?eo [nicaaou oaee aaoiiaoe?anee] eee aaanoe a iiea eiy oaeea!'",
"SAVING TO"=>"NIO?AIEOU A",
"Sorry, access denied"=>"Ecaeieoa, ainooi cai?auai",
"for example, uncomment next line"=>"aey i?eia?a, ?aneiiiaioe?oeoa neaao?uo? no?ieo",
"Eval PHP code"=>"Auiieieou PHP eia",
"don't type"=>"ia ieoeoa",
"and"=>"e",
"example (remove comments '#')"=>"i?eia? (oaaeeoa eiiiaioa?ee '#')",
"Shell commands"=>"Eiiaiau Shell'a",
"filesize to 0byte"=>"?acia? a 0 aaeo",
"from"=>"io",
"to"=>"a",
"Full file name"=>"Iieiia eiy oaeea",
"Can't open directory"=>"Ia iiao ioe?uou eaoaeia",
"setup"=>"iano?ieea",
"back"=>"iacaa",
"Reset all settings"=>"Na?ineou ana iano?ieee",
"clear"=>"i?enoeou",
"Current"=>"Oaeouea",
"Colums and sort"=>"Eieiiee e ni?oe?iaea",
"Sort order"=>"Ii?yaie ni?oe?iaee",
"Ascending sort"=>"Ii aic?anoaie?",
"Descending sort"=>"Ii oauaaie?",
"Sort by filename"=>"Ni?oe?iaaou ii eiaie oaeea",
"Sort by filename extension"=>"Ni?oe?iaaou ii ?anoe?aie? oaeea",
"Date/time format"=>"Oi?iao aaou/a?aiaie",
"Panel font & size"=>"O?eoo/?acia? iaiaee",
"Setup"=>"Iioee",
"Char map"=>"Neiaieu",
"Language"=>"?cue",
"English"=>"Aiaeeeneee",
"Russian"=>"?onneee",
"Character map (symbol codes table)"=>"Oaaeeoa neiaieia",
"Select font"=>"Auaa?eoa o?eoo",
"or type other"=>"eee aaaaeoa a?oaie",
"Font size"=>"?acia? o?eooa",
"Code limit"=>"Aeiacii eiaia",
"Generate table"=>"Naaia?e?iaaou oaaeeoo",
"Universal convert"=>"Oieaa?naeuiua eiiaa?oaoee"
);/*--mmstop--*/
...
<input type=submit><br>";
   ...
   $russtr2="EOOEAIACOUOUAAI?IEAYNIEOUAeooeaiacououaai?ieaynieoua";
   function from_translit($ss) {
      global $russtr1,$russtr2;
      $w=array("Sch",'U',"SCH",'U',"ScH",'U',"SCh",'U',"sch",'u',"Jo",'?',"JO",'?',"jo",'?',
      "Zh",'?',"ZH",'?',"zh",'?',"Ch",'?',"CH",'?',"ch",'?',"Sh",'O',"SH",'O',"sh",'o',
      "##",'U',"''",'U',"Eh",'Y',"EH",'Y',"eh",'y',"Ju",'?',"JU",'?',"ju",'?',"Yu",'?',
      "YU",'?',"yu",'?',"YA","?","Ya","?","ya","y","Ja",'?',"JA",'?',"ja",'y');
      $c=count($w);
      for ($i=0; $i<$c; $i+=2) $ss=str_replace($w[$i],$w[$i+1],$ss);
      $ss=strtr($ss,$russtr1,$russtr2);
      $ss=preg_replace("!([a-y]+)~([a-y]+)!is","\\1\\2",$ss);
      return $ss;
   }
