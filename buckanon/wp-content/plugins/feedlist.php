...
//"http://del.icio.us/rss/bdefore/buckanon",15,true,false,'<div class="entry"><p>','</p></div>',' '

		// Default settings:
		$rss_feed_url = 'http://del.icio.us/rss/bdefore/buckanon';
		$num_items = 15;
			...
			if (count($items))
			{
				...
				***********************/
				...
				foreach ($items as $item)
				{
					...
					// magnolia stupidly puts a p tag in the description - remove them
					//$thisDescription = str_replace("<p>","",$thisDescription);
					//$thisDescription = str_replace("</p>","",$thisDescription);
					//$thisDescription = str_replace("*/*","<a href=\"",$thisDescription);
					//$thisDescription = str_replace("*\*","\">",$thisDescription);
					//$thisDescription = str_replace("*_*","</a>",$thisDescription);
...
//						$rssList .= '<div class="ReadMoreLink"><a href="'.htmlentities(utf8_decode($item['link'])).'">'.$settings["translations"][$settings["language"]]['ReadMore'].'</a> &nbsp; </div>';
						$rssList .= '<div class="ReadMoreLink"><a href="'.$item['link'].'">'.$settings["translations"][$settings["language"]]['ReadMore'].'</a> &nbsp; </div>';
					 }
		...
		// added this since entities were sneaking into the feed
		$rssList = str_replace("&lt;","<",$rssList);
		$rssList = str_replace("&gt;",">",$rssList);
		$rssList = str_replace("&quot;","\"",$rssList);
		$rssList = str_replace("&amp;","&",$rssList);
